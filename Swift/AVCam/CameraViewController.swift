...
class CameraViewController: UIViewController, AVCaptureFileOutputRecordingDelegate, ItemSelectionViewControllerDelegate {
    ...
    override var keyCommands: [UIKeyCommand]? {
        ...
        for number in numbers {
                let strNumber = number as String
                if Int(strNumber) == nil {
                    commands.append(UIKeyCommand(action: #selector(keyPressed(key:)), input: strNumber))
                } else {
                    commands.append(UIKeyCommand(action: #selector(keyPressed(key:)), input: strNumber))
                }
            }
    ...
    @objc private func keyPressed(key: UIKeyCommand) {
        guard let pressingNumber = key.input else {return};
        ...
        print("you pressed number " + pressingNumber)
        ...
        sessionQueue.async {
            ...
            // Specify save the location, time and pressed number when photo was taken
            photoCaptureProcessor.location = self.locationManager.location
            photoCaptureProcessor.number = pressingNumber
            photoCaptureProcessor.dateAndTime = currentDateString
